...
FROM employees JOIN offices ON employees.officeCode = offices.officeCode WHERE offices.city = 'San Francisco'
...
FROM products WHERE `productLine` LIKE '% Cars' ORDER BY `productLine` DESC, SUBSTRING(productName, 6) ASC;
